...
#' Generate html report about differential sites for a given RiboClass.
#' @inheritParams report
#' @export
report_diff_sites <- function(ribo, condition_col, project_name = "Unnamed project", output_dir = getwd()) {
  ribo_name <- deparse(substitute(ribo))
  ...
  report(ribo, ribo_name,"diff_sites.Rmd",condition_col = condition_col,
         library_col = NULL,
         project_name = project_name,
         output_dir = output_dir)
}

#' Generate html report about 2'ome sites for a given RiboClass.
#' @inheritParams report
#' @export
report_2ome_sites <- function(ribo, condition_col, project_name = "Unnamed project", output_dir = getwd()) {
  ribo_name <- deparse(substitute(ribo))
  ...
  report(ribo, ribo_name,"2ome_analysis.Rmd",condition_col = condition_col,
         library_col = NULL,
         project_name = project_name,
         output_dir = output_dir)
}
