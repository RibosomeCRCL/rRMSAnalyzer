...
.plot_RLE <- function(mat = NULL,show_outlier,col_to_plot) {
  ...
  rle.plot <- ggplot(rle.calc, aes(x = key, value)) + geom_boxplot(outlier.shape =outlier_shape ,outlier.size = 0.3, fill = "red") + ylab(paste0("log2(",col_to_plot,"/median)")) + xlab("Sample ID") + ggtitle("RLE plot") + theme(axis.text.x = element_text(angle=90 )) +  geom_hline(yintercept = 2*mad(rle.calc$value, na.rm = T), colour = "blue") + geom_hline(yintercept = - 2*mad(rle.calc$value, na.rm = T), colour = "blue")
  return(rle.plot)
}
