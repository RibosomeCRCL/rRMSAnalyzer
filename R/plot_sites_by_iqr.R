...
plot_sites_by_IQR <- function(ribo = NULL, plot = "IQR", variance = "IQR", ...) {
  ...
  IQR_order_df <- IQR_order_df[order(IQR_order_df[,1], decreasing = TRUE), ]
  ...
  assign("median_2mad", median_2mad, envir = .GlobalEnv)
  ...
  if (tolower(plot) %in% c("iqr", "var")) {
    ...
    p1 <- ggplot(IQR_order_df, aes_string(x = "site", y = tolower(variance))) +
      ...
      ggtitle("Scatter plot of IQR values across samples") +
      theme_bw() + 
      ...
      labs(x = "rRNA 2'Ome sites",
           ...
           subtitle = paste(ncol(Cscore_matrix), "samples and", nrow(Cscore_matrix), "positions")) +
      theme(...) +
  ...
  if (tolower(plot) == "boxplot") {
    p1 <- ggplot(IQR_order_df, aes(x = site, y = cscore)) +
      ...
      labs(x = "Scatter plot of IQR values across samples",
           y = "Cscore", size = 20) +
      theme(...)
    return(p1)
  }
