...
part1 <- ggplot(metadata_data_total, aes(x = site, y = difference, label = site)) +
  ...
  scale_fill_manual(
    ...
    drop = FALSE # keep factor level in legend even if they are not present in data
  ) +
...
# combine graphs
  plot_comparison_median <- patchwork::wrap_plots(part1, part2, widths = c(2,1))

  return(plot_comparison_median)
}
