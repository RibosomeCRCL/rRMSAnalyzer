...
#'
#' @examples
plot_heatmap_corr <- function(ribo,col_to_plot,cols_for_annotation, order_by_col) {
  
  matrix <- aggregate_samples_by_col(ribo[["raw_counts"]],col_to_plot,position_to_rownames = T)
  .plot_heatmap_corr(matrix,ribo[["metadata"]],order.by.col=order_by_col,cols_for_annotation = cols_for_annotation)
...
#'
#' @examples
.plot_heatmap_corr <- function(cscore.matrix = NULL, annotation.samples = NULL,cols_for_annotation = NULL ,order.by.col = NULL) {
