...
render_html_wilcoxon <- function(ribo, condition_col, ctrl, cases, pthr, cscore_cutoff) {
  ...
  res_pv <- rRMSAnalyzer::res_pv(ribo = ribo_filtered, test = "wilcoxon", condition_col = condition_col)
  ...
  x <- rRMSAnalyzer::plot_stat(ribo = ribo_filtered, site = NULL, res_pv = res_pv, pthr = pthr, condition_col = condition_col, cscore_cutoff = cscore_cutoff)
