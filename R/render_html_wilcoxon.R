...
#' render_html_wilcoxon(ribo_toy, "condition", ctrl, cases, pthr = 0.05, cscore_cutoff = 0.05)
render_html_wilcoxon <- function(ribo, condition_col, ctrl, cases, pthr, cscore_cutoff) {
  ribo_filtered <- ribo
  ...
  compute_pval <- rRMSAnalyzer::compute_pval(ribo = ribo_filtered, test = "wilcoxon", condition_col = condition_col)
  ...
  x <- rRMSAnalyzer::plot_stat(ribo = ribo_filtered, site = NULL, compute_pval = compute_pval, pthr = pthr, condition_col = condition_col, cscore_cutoff = cscore_cutoff)
  empty_x <- list(plot_env = list(significant_sites = character(0)))
  # Conditionnal text
  text <- if (length(x[["plot_env"]][["significant_sites"]]) != 0) {
    glue::glue("This panel displays boxplots of C-scores for rRNA 2'O-methylation 
    (2'Ome) sites identified as significantly different between the two biological 
    conditions {cases} and {ctrl}, based on Wilcoxon rank-sum test results. 
    Significance was defined by the criteria: adjusted p-value < 0.05 and 
    |deltaC-score| > 0.05, where deltaC-score = Median C-score_{cases} - Median C-score_{ctrl}.")
  } else {
    glue::glue("No significantly differentially methylated rRNA 2'O-methylation (2'Ome) 
         sites were identified under the current threshold (Wilcoxon rank-sum test,
         adjusted p-value < 0.05 &  |deltaC-score| > 0.05).")
  }
  ...
  if (length(x[["plot_env"]][["significant_sites"]]) != 0) {
    ...
    return(list(
      ui = htmltools::tagList(output_list), plot = empty_x))
  }
}