...
.check_sample_positions <- function(sample_1, sample_2, sample_1_name, sample_2_name) {
  ...
  if (length(sample_2[, "named_position"]) == sample_size) { # We compare if sample_2 has the same number of positions as sample_1
    if (sum(sample_1["named_position"] == sample_2["named_position"]) == sample_size) # counts the number of identical positions 
      return(TRUE) # If all positions are identical (sum == sample_size) returns TRUE
    else {
      errored_positions_file_1 <- setdiff(sample_1[["named_position"]], sample_2[["named_position"]]) #Find the elements of sample_1 that are not in sample_2
      errored_positions_file_2 <- setdiff(sample_2[["named_position"]], sample_1[["named_position"]]) #Find the elements of sample_2 that are not in sample_1
  ...
  else { # otherwise we return an error message
