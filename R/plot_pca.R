...
#' @param axis two-element vector indicating which pair of principal components you want to show.
...
plot_PCA <- function(ribo, color_col = NULL, axis = c(1,2), only_annotated=F, pca_object_only = F) {
  
  
  if (is.null(ribo)) {stop("MISSING parameter: please provide a RiboClass!")}
  if (is(ribo) != "RiboClass") {stop("ribo parameter is not a RiboClass!")}
  ...
  facto_pca <- .plot_pca(pca_calculated,ribo[["metadata"]],color_col, axis = axis)
...
#' @return
#'
.plot_pca <- function(dudi.pca = NULL, metadata = NULL, color_col = NULL, axis = axis) {
  ...
  plot.pca <- factoextra::fviz_pca_ind(dudi.pca, 
    ...
    ggplot2::ylab(paste("PC", axis[2],":", round(dudi.pca$eig[axis[2]]/sum(dudi.pca$eig) * 100, 1), "%")) +
    ggplot2::xlab(paste("PC", axis[1],":", round(dudi.pca$eig[axis[1]]/sum(dudi.pca$eig) * 100, 1), "%"))
