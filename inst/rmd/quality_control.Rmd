...
#   ggplot2::geom_bar(stat = "identity", width = 0.1) + # Ajuster la largeure de la barre 
...
# grid.arrange(plot_counts_fraction, theorical_counts_plot, ncol = 2, widths = c(3, 1))
```
...
```{r heatmap_corr}
#plot_heatmap_corr <- rRMSAnalyzer::plot_heatmap_corr(ribo_to_check,"Count")
```



```{r heatmap_v2, fig.asp = 0.9, fig.width = 15}
# TEST New heatmap with outlier_level
...
row_annotation <- HeatmapAnnotation(
  ...
  col = list(
    coverage_quality = c("ok" = "darkgrey", "warning" = "red"),
    rle_median_quality = c("ok" = "darkgrey", "warning" = "red")
  ),
  annotation_legend_param = list(
    title_gp = gpar(fontsize = 16), # Taille du titre
    labels_gp = gpar(fontsize = 14), # Taille des étiquettes
    grid_width = unit(8, "mm"),     # Largeur des carrés
    grid_height = unit(8, "mm")     # Hauteur des carrés
  )
)

#créer un vecteur pour la couleur des échantillons suivant le niveau d'outlier 
column_colors <- ifelse(qcdata$total_outliers > 0, "red", "black")

# Identifier les échantillons en "Warning"
column_colors <- ifelse(
  qcdata$coverage_quality == "warning" | qcdata$rle_median_quality == "warning",
  "red",  # Échantillons "Warning" en rouge
  "black" # Autres échantillons en noir
)

# Plot de la heatmap avec noms des colonnes colorés
x <- Heatmap(
  ...
  heatmap_legend_param = list(
    legend_direction = "horizontal", # Légende horizontale
    title_gp = gpar(fontsize = 16),  # Taille du titre de la légende
    labels_gp = gpar(fontsize = 14), # Taille des étiquettes de la légende
    grid_height = unit(10, "mm"),    # Hauteur des carrés de la légende
    grid_width = unit(10, "mm")      # Largeur des carrés de la légende 
  ),
  ...
  column_names_gp = gpar(col = column_colors, fontsize = 16) # Couleur des noms des colonnes
)

draw(
  x,
  heatmap_legend_side = "bottom",  # Position de la légende
  annotation_legend_side = "bottom" # Pour les légendes des annotations
)
```

## Correspondance analysis {.tabset}
...
plot_coa <- rRMSAnalyzer::plot_coa(ribo_to_check,color_col = params$library_col)+ 
  scale_color_manual(values = c("L1" = "#00FFFF", "L2" = "orange", "L3" = "purple", "L4" = "#808000", "L5" = "#000080", "L6" = "pink", "L7" = "#FF00FF", "L8" = "#CCCCFF", "L9" = "#9FE2BF", "L10" = "#DFFF00", "L11" = "#DFAF00", "L12" = "#DFDF00"))
...
# library(plotly)
# plotly(plot_coa)
#grid.arrange(plot_coa, plot_coa_binaire, ncol = 1)
```
...
#plot_coa + plot_coa_binaire + plot_layout(widths = c(2,1))
```
