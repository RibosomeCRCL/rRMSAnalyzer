...
```{r relative log cov}
rRMSAnalyzer::plot_rlc(ribo_to_check, show_outlier = TRUE) + ggplot2::ggtitle("Relative Log Coverage")
```
...
<br>
Outlier detection steps identified **`r sum(qcdata$coverage_quality != "pass")`**  sample(s) based on the global coverage distribution, and **`r sum(qcdata$rlc_median_quality == "warning")`** sample(s) as a potential outlier(s) using the Relative Log Coverage (RLC) metric. The sample(s) identified as outlier(s) based on QC metrics should be interpreted with caution in downstream analyses :

```{r print outlier samples}
if (any(qcdata$total_outliers != 0)) {
outlier_summary <- data.frame(outliers = qcdata$samplename[qcdata$total_outliers != 0]) # print samples that is outlier at least 1 time
} else  outlier_summary <- "No outlier"
