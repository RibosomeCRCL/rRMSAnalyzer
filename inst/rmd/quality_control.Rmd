...
The coverage distribution of absolute end read counts at each genomic position is visuallized here using a boxplot for each individual samples, controlling for coverage variation. 
...
The following image shows a stacked barplots representing end read count fractions of total end read counts for each RNA per sample. Theoretical mean fractions have been computed using 38 samples issued from 6 different human cell lines and the commercial Human XpressRef Universal Total RNA (Qiagen). Theoretical mean fractions are given as dotted lines. In the case of rRNAs, most end read counts are assigned to the longest rRNA, namely the 18S (~ 0.30) and the 28S (~ 0.60).
...
*Comments can be written her to appear on the html document*
