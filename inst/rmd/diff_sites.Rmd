...
#est-ce qu'on veut vraiment faire ce line plot sachant qu'il dit quasiement la meme chose que le suivant ?? 

library(ggplot2)
 
# Get the data from the web !
# ctrl <- read.table("http://www.sr.bham.ac.uk/~ajrs/papers/sanderson06/mean_Tprofile-CC.txt" ,  header=TRUE)
# t <- read.table("http://www.sr.bham.ac.uk/~ajrs/papers/sanderson06/mean_Tprofile-nCC.txt" , header=TRUE)
# CC$type <- "Cool core"
...
# Make the plot
ggplot(data=ribo, aes(x=named_position, y=`N3-GT2-JFH`, ymin=0, ymax=1.0, fill=`RNA-ref-2`, linetype=`RNA-ref-2`)) + 
 geom_line() + 
...
stats <- ribo %>%
  group_by(named_position, condition) %>%
  summarise(
    median = median(c_score),
    q25 = quantile(c_score, 0.25),
    q75 = quantile(c_score, 0.75)
  )

# Tracé
ggplot(stats, aes(x = named_position, y = median, linetype = condition, fill = condition)) +
  geom_line() +
...
# prend en entrée une riboclass annotée
