...
new_metadata_sites <- ribo_to_check%>%
  pivot_longer(cols = c("L1-CTA", "L1-SNORD104", "L2-CTA", "L2-SNORD104"), names_to = "sample_name", values_to = "cscore") %>% #remplacer les noms des échantillons, ici "L1-CTA/SNORD104" 
...
metadata_data_total <- new_metadata_sites%>%
  group_by(site) %>%
  summarize(median_Cscore_metadata_ech_ctrl = median(cscore[sample_name == c("L1-CTA", "L2-CTA")]), # indiquer le nom de l'échantillon 
            #si 1 seul échantillon == "samplename" / si >1 échantillon == c("samplename1", "samplename2")
            median_Cscore_metadata_ech = median(cscore[sample_name == c("L1-SNORD104", "L2-SNORD104")]), # indiquer le nom de l'échantillon
            #si 1 seul échantillon == "samplename" / si >1 échantillon == c("samplename1", "samplename2")
...
part1 <- ggplot(metadata_data_total, aes(x=site, y=difference, label=site)) +
  ...
  labs(subtitle="Difference of median C-score (LNA-SNORD - LNA-CTL)", #à changer
       title= "Figure") +
...
part2 <- ggplot(metadata_data_total) +
  ...
  geom_point(data = metadata_data_total[,c(1,3)],
             aes(x = site, y = median_Cscore_metadata_ech, color = "LNA-SNORD"), 
             size = 2) +
  geom_point(data = metadata_data_total[,c(1,2)],
             aes(x = site, y = median_Cscore_metadata_ech_ctrl, color = "LNA-CTA"),
             size = 2) +
  theme_bw() +
  theme(axis.title.y = element_blank(),
        ...
        plot.subtitle = element_text(hjust = 0.5),
        panel.grid.minor = element_blank()) +
  geom_segment(data = metadata_data_total[which(metadata_data_total$site == "28S_Cm1327"), ], 
               #changer les noms des sites d'intérêt
               #si 1 site: [which(metada_data_total$site == "28S_Cm1327"), ]
  ...
  scale_color_manual(values = c("ech" = "cyan", "ech_ctrl" = "gray"),
                     name = "Légende")
