...
ggplot(df_of_Cscores, aes(x = site, y = median_cscore, group = group.id, color = group.id, fill = group.id)) +
  geom_line() +
...
df_filtered <- df_of_Cscores[df_of_Cscores$group.id == "MOCK-DCV", ]
...
ggplot(df_of_Cscores, aes(x = site, y = median_cscore, group = group.id, color = group.id, fill = group.id)) + # remplacer df_filtered par df_of_Cscores
  ...
  theme_bw() +
  ggtitle("C-score par groupe avec quartiles")
  #facet_wrap(~ group.id, scales = "free_y") # séparer ou non les échantillons
