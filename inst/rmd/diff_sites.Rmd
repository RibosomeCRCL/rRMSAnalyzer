...
## Site-by-site analysis of rRNA 2'Ome // on garde cette partie comme elle est.

The alteration in rRNA 2'Ome is analysed site-by-site individually to identify significant alteration in rRNA 2'Ome level at particular site depending on the biological context. c'est le line plot. 
...
ribo_long <- ribo_df %>%
  pivot_longer(cols = -site, names_to = "condition", values_to = "c_score") 


# Filtrer pour les deux conditions souhaitées
ribo_filtered <- ribo_long %>%
  filter(condition %in% c("N3-GT2-JFH", "N1-GT1-TN-Hi"))

# Calculer les statistiques pour les quartiles
stats <- ribo_long %>%
  group_by(site, condition) %>%
  summarise(
    median = median(c_score, na.rm = TRUE),
    q25 = quantile(c_score, 0.25, na.rm = TRUE),
    q75 = quantile(c_score, 0.75, na.rm = TRUE),
    .groups = "drop"
  )

# Tracer le graphique
ggplot(stats, aes(x = site, y = median, color = condition, fill = condition)) +
  geom_line(size = 1) +  # Lignes
  geom_ribbon(aes(ymin = q25, ymax = q75), alpha = 0.3) +  # Ruban pour les quartiles
  scale_x_discrete(expand = c(0, 0)) +  # Échelle x basée sur les sites (discrets)
  scale_y_continuous(limits = c(0, 1), expand = c(0, 0)) +  # Limites pour y
  labs(
    x = "Position (site)",
    y = "C-score",
    color = "Condition",
  ...
  theme(
    ...
    axis.text.x = element_text(angle = 45, hjust = 1)  # Rotation des labels des positions
  )
...
# 1. Extraction des données de la RiboClass
ribo_df_test2 <- as.data.frame(ribo_adj_annot[[1]])  # Transforme RiboClass en data frame
metadata <- ribo_adj_annot[[2]]  

ribo_df_test2 <- extract_data(ribo_adj_annot, "cscore", position_to_rownames = TRUE)

# Ajout des métadonnées aux colonnes du ribo_df_test2
colnames(ribo_df_test2)[-1] <- metadata$condition # marche pas, il faut trouver une solution pour ressortir les conditions ~ cscores ~ position

# 2. Transformation en long format et agrégation par groupe
ribo_long_test2 <- ribo_df_test2 %>%
  pivot_longer(-site, names_to = "condition", values_to = "c_score") %>%
  left_join(metadata, by = c("condition" = "condition")) %>%
  group_by(site, Groupe) %>%
  summarise(
    median_cscore = median(c_score, na.rm = TRUE),
    q25 = quantile(c_score, 0.25, na.rm = TRUE),
    q75 = quantile(c_score, 0.75, na.rm = TRUE),
    .groups = "drop"
  )
...
ggplot(ribo_long_test2, aes(x = site, y = median_cscore, group = Groupe, color = Groupe, fill = Groupe)) +
  geom_line() +
  geom_ribbon(aes(ymin = q25, ymax = q75), alpha = 0.3) +
  scale_x_discrete(guide = guide_axis(angle = 90)) +  # Rotation des étiquettes si besoin
  scale_y_continuous(limits = c(0, 1)) +
  xlab("Position") +
  ylab("C-score") +
  theme_bw() +
  ggtitle("C-score par groupe avec quartiles")
```

Here is given an histo plot given the mean C-score and the standard deviation in the different biological context for each individual rRNA 2'Ome sites. Color indicates the different biological groups. Plot de Julie Herleinmecker
```{r}
library(knitr)
library(ggplot2)
library(cowplot)
