...
rRMSAnalyzer is an R package that provides a set of easy-to-use functions to compute C-scores from RiboMethSeq read end counts as input, adjust batch effect with ComBat-Seq, visualize the data and provide a table with the annotated human rRNA sites and their C-scores.

> **Note**
> We have also developed a [dedicated Nextflow pipeline](https://github.com/RibosomeCRCL/ribomethseq-nf) to process the data from sequencing output (fastq files) to useful raw data for rRMSAnalyzer (read end counts).
...
- [RNA annotation](#rna-annotation)
  ...
  * [Annotation of RNA 2'Ome sites](#annotation-of-rna-2ome-sites)
    + [Annotation included: Human 2'Ome rRNA sites](#annotation-included--human-2ome-rrna-sites)
