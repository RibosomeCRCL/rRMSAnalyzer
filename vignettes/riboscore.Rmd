...
title: "riboscore"
abstract: > 
  RiboMethSeq is an innovative RNASeq-based approach that has been developed in 2015 to quantify rRNA 2'Ome at all the sites at once in yeast. RiboMethSeq is based on a partial alkaline hydrolysis. Since presence of 2’Ome on a nucleotide n protects its bond with the following n+1 nucleotide, it induces a drop in the number of reads starting at the n+1 position compared to the local coverage and thus allowing to calculate a 2’Ome level.
  <br>Using 5'end count data as input, the package Riboscore provides a set of easy-to-use functions to compute c-score, adjust batch effect with ComBat-Seq and visualise your data with several plots.
output:
...
[Only two columns are necessary for your metadata :]{.underline}
...
The sample should not be grouped by library or batch. Unfortunately, this is clearly visible on PC 1.

We can fix this with ComBat-Seq. Riboscore has a wrapper to use this correction on a RiboClass : adjust_bias.

```{r}
ribo_adjusted <- adjust_bias(ribo,"run")
```

This will create a new RiboClass with adjusted count values. The c-score will be recomputed using the previous parameters given.

Now, if we redo the PCA :

```{r, fig.width=5,fig.height=5}
plot_PCA(ribo_adjusted,"run")
```

The bias on PC 1 is no longer there !
...
in both cases, only the remaining samples' metadata are kept in the RiboClass object. You do not need to update by hand.

# Data exploration

## Rename RNA

For some plots or functions, the RNA names can be important. This particulary the case for site annotation.
...
cat("ribo's rna names :",ribo_adjusted$rna_names$current_name)
```
...
```{r}
ribo_adjusted <- rename_rna(ribo_adjusted, c("5S","5.8S","18S","28S"))

```

## Annotate sites {#annotate-sites}

Among the thousands sites belonging to the rRNA, some of them are more particular than others. In our case, we are interested by sites that are known to be 2'O-methylated in the human model.
...
By default, the axes for your PCA plot are PC 1 and PC 2. **The *axes* parameter lets you choose which principal component you want to plot.** Using our previous example, we will plot this time PC 1 and PC 3 :
...
**If you want a ggplot-ready dataframe**, you can use format_to_plot. With the RiboClass, you can add additionnal metadata column if necessary.

```{r}
ggplot_table <- format_to_plot(ribo_adjusted,"condition")
knitr::kable(ggplot_table[501:510,])
```
