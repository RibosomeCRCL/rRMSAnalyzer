...
The 2’Ome sites of interest must be attached to the RiboClass object for further analysis using the `annotate_site()` function with either the provided annotations or custom annotations.
...
The `plot_stat()` function produces a boxplot visualizing the median C-scores per biological condition and the p-value. But it uses a res_pv table that can be generated from ANOVA, Welch, Kruskal or Wilcoxon test thanks to the `res_pv()` function. These functions are described below. To use them a RiboClass needs to be loaded with annotated sites. 

```{r}
data("ribo_toy")
ribo_adjusted <- adjust_bias(ribo_toy,"run") # optional

data("human_methylated")
cat("human_methylated's rna names: ", unique(human_methylated$rRNA),"\n")
cat("ribo's rna names: ", as.character(ribo_adjusted$rna_names$current_name)) # change name of RiboClass if necessary
ribo_adj_name <- rename_rna(ribo_adjusted, # rename RNA if needed
                             new_names = c("5S", "5.8S", "18S", "28S"))
