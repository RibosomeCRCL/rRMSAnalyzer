...
Due to technical limitations, it is sometimes necessary to make the wet-lab preparation of large cohort in several batches.
The main risk when making several batches is to introduce **technical biases** or **batch effect** in the dataset.

## Quality control report 

A quality control (QC) report can be performed. It uses several metrics to help identify outlier sample and/or batch effects, including end read counts and the C-score itself at all the 7217 genomic positions. The QC verifies that the coverage is uniform and reproducible between samples, eliminating the possibility of biases due to sequencing and outlier samples. 
...
The C-score depends on the local coverage around the site of interest. For a simple visualization of the read end count coverage of the flanking region of a given genomic position of interest, use the plot_counts_env function. 
...
The rRMSAnalyzer package calculates a C-score for each genomic position of the RNAs. However, not every sites of the reference 2’Ome RNA is necessarily methylated. Therefore, it is expectated that the user provide a list of potentially methylated sites of interest, called “annotated sites”. This will make it easy to restrict further analysis to this list of sites.
...
Instead of using the list of human rRNA 2’Ome sites provided with rRMSAnalyzer package, the user can provide his own list of annotated sites which will be attached to the RiboClass object using the annotate_site function (see 10.3 section : annotation of RNA sites).

This annotate_site function expects an “annot” parameter which must be an object of type  dataframe that contains the following three mandatory columns :
...
To determine whether RNA 2’Ome profiles are different between conditions and identify the most variable RNA 2’Ome sites, several functions have been implemented to return a ggplot. Moreover, the parameter only_annotated, which is included in all the plot related functions, allows to plot only the annotated RNA 2’Ome sites of interest (i.e., with biological relevance) when returned as True.
...
**Note 2**  the *object_only* parameter returns the complete dudi.pca object by setting *object_only* to TRUE:
...
Two cut-offs are currently used to identify RNA sites with  significantly different 2’Ome level between biological conditions (Marcel et al , NAR Cancer 2021):

-   statistical significance: a p-value < 0.01 using Kruskal-Wallis test. 
-   biological significance: a ΔC-score (i.e., Median C-score~max~ – Median C-score~min~) > 0.05, indicating a variation of 5% of the RNA 2’Ome level between the conditions.
...
The *mean_samples_by_condition* function returns the summarized values per modality of a variable in the metadata dataframe.

By default, the function returns for all the genomic RNA positions the name of the position, the mean and standard deviation (sd) of the C-scores. By using the value=”count” parameter, it is als possible to calculate these values from the read end counts. Calculations can be limited to only annotated RNA 2’Ome sites by setting the only_annotated parameter to TRUE.
